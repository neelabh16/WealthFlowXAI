...
export function WealthSweepControl({
  rule,
  contributions,
  onChanged,
}: {
    ...
    <Card>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="font-display text-2xl text-[color:var(--text-strong)]">Route actual round-ups and cashback into wealth</h2>
        ...
        <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface-strong)] px-4 py-3 text-sm text-muted">
          Provider: {rule.provider === "mock" ? "Local mock funding" : "Razorpay"}
          <br />
          {rule.provider_ready ? "Ready to accept sweep actions." : "Configure Razorpay keys before enabling real checkout."}
        </div>
      </div>
      ...
      <div className="mt-6 space-y-4">
        {contributions.map((contribution) => (
          <div key={contribution.id} className="surface-panel flex flex-col gap-4 px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
